...
// File: contracts/tokens/LockCoin.sol
...
contract LockCoin is ERC20Capped, MintableBaseToken {
    ...
    mapping(address => TimeLockByAddress) public mappingAddressToLock;

    constructor(
        string memory name,
        string memory symbol,
        uint256 totalSupply
    ) ERC20Capped(MAX_SUPPLY) MintableBaseToken(name, symbol, totalSupply) {
    ...
    function _transferLocker(address newLocker) internal {
        ...
        require(newLocker != locker, "New locker is same as previos locker");
        locker = newLocker;
